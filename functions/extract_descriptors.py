...
def filter_keypoints(img, keypoints, patch_size = 9):
    # TODO: Filter out keypoints that are too close to the edges of the image
    bound = int(patch_size / 2)
    mask = (keypoints[:, 0] >= bound) & (keypoints[:, 0] < img.shape[0] - bound) & (keypoints[:, 1] >= bound) & (keypoints[:, 1] < img.shape[1] - bound)
